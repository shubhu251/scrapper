"""
Spider for scraping Yali Dream Creations website (https://www.yalidreamcreations.com/)
...
class YaliDreamCreationsSpider(BaseComicSpider):
    """
    Spider to scrape Yali Dream Creations website.
    Extracts publisher information, comics, series, and artist data.
    
    Usage:
        scrapy crawl yali_dream_creations
    """
    
    name = 'yali_dream_creations'
    allowed_domains = ['yalidreamcreations.com']
    start_urls = ['https://www.yalidreamcreations.com/shop/']
    ...
    def extract_publisher_info(self, response):
        ...
        item = PublisherItem()
        item['name'] = 'Yali Dream Creations'
        item['website'] = 'https://www.yalidreamcreations.com'
        item['url'] = 'https://www.yalidreamcreations.com'
    ...
    def parse_product_detail(self, response):
        ...
        try:
            ...
            # Extract publisher
            item['publisher'] = 'Yali Dream Creations'
            ...
            # Strategy 1: Get all paragraphs from description tab panel
            ...
            if item.get('title'):
                ...
                if series_match:
                    ...
                    invalid_suffixes = [
                        ...
                        r'\s+Hardcover\s*$',
                        r'\s+Paperback\s*$',
                        r'\s+Variant\s+Cover\s*$',
                        r'\s+Variant\s*$',
                        r'\s+Regular\s+Cover\s*$',
                        r'\s+English\s+Hardcover\s*$',
                        r'\s+Hindi\s+Hardcover\s*$',
                        r'\s+English\s+Paperback\s*$',
                        r'\s+Hindi\s+Paperback\s*$',
                        r'\s+Box\s+Set\s*$',
                    ...
                    invalid_series_values = [
                        ...
                        'Regular Cover', 'Box Set', 'Deluxe Edition', 'Book', 'Vol', 'Volume', 'Stage'
                    ]
                    ...
                    if issue_match:
                        ...
                        # Try to extract number from title (e.g., "Book 1", "Book 2")
                        book_match = re.search(r'Book\s+(\d+)', item['title'], re.IGNORECASE)
                        if book_match:
                            try:
                                item['issue'] = int(book_match.group(1))
                            except ValueError:
                                pass
                        else:
                            # Try to extract from "Vol. 3"
            ...
            language_match = re.search(r'\b(Hindi|English|हिंदी|Eng|Hin|Malayalam)\b', title_text, re.IGNORECASE)
            if language_match:
            ...
            if not language:
                language_match = re.search(r'\b(Hindi|English|हिंदी|Eng|Hin|Malayalam)\b', desc_text, re.IGNORECASE)
                if language_match:
                    lang = language_match.group(1)
                    if lang.lower() in ['hindi', 'hin', 'हिंदी']:
                        language = 'Hindi'
            ...
            # Strategy 1: Check additional_info section first
            ...
            # Try to extract from CSS selectors first
            ...
            # Extract genre - set to empty array for Yali Dream Creations
            item['genre'] = []
        ...
        if item and item.get('series'):
            try:
                ...
                series_item['publisher'] = 'Yali Dream Creations'

        ...
        try:
            ...
            if additional_info_section:
                ...
                if pages_text and not item.get('pages'):
                    try:
                        pages_value = int(pages_text)
                        if 20 <= pages_value <= 500:  # Reasonable range
                            item['pages'] = pages_value
                    ...
                    if not page_count:
                        ...
                        for word in reversed(words[-3:]):
                            ...
                            if clean_word.isdigit():
                                num = int(clean_word)
                                # Reasonable page count range for comics (20-500)
                                if 20 <= num <= 500:
                                    page_count = num
                                    break
                    
                    # Pattern 3: Look for any 2-3 digit number in description (fallback)
                    if not page_count:
                        numbers = re.findall(r'\b(\d{2,3})\b', desc_text)
                        if numbers:
                            # Filter reasonable page counts (between 20 and 500)
            ...
            if not page_count:
                ...
                page_match = re.search(r'(?:pages?|pgs?)[:\s]+(\d{2,3})\b', full_text, re.IGNORECASE)
                if page_match:
                    try:
